...
def get_msgList():
    # Retrieves message ID list from 'orders' label
    try:
        # Call the Gmail API
...
def get_msgContent(message_id):
    # Retrieves message body and header, decodes with base64
    try:
        service = build('gmail', 'v1', credentials=creds)
...
# -- Main program loop --
# We create a workbook, with each worksheet containing the client's name + order details

workbook = xlsxwriter.Workbook(Path.home() / 'Desktop' / 'OrderBook.xlsx')
for msg_id in get_msgList():

    total = 0           # Total price
    row = 5             # Row and Col starting values
    col = 1
    ...
    for msg_url in msg_body:
        if 'https:' not in msg_url:
            ...
            # Write values in corresponding cells
            ...
            worksheet.write_url(row, col, msg_url)
